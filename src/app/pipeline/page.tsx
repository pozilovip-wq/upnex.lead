...
import { PIPELINE_STAGES, PipelineStage } from '@/lib/data'
import { cn, getLeadScoreColor, getInitials } from '@/lib/utils'
import { GripVertical, Brain, Plus } from 'lucide-react'

export default function PipelinePage() {
  const { students, moveStudent } = useStore()
  const [dragging, setDragging] = useState<string | null>(null)
  const [dragOver, setDragOver] = useState<string | null>(null)
  const [showModal, setShowModal] = useState(false)

  const handleDrop = (stage: PipelineStage) => {
    if (dragging) moveStudent(dragging, stage)
    setDragging(null)
    setDragOver(null)
  }

  const stageColors: Record<string, string> = {
    'New Lead': 'border-slate-300', 'Contacted': 'border-blue-300',
    'Consultation Scheduled': 'border-indigo-300', 'Documents Requested': 'border-purple-300',
    'Documents Received': 'border-violet-300', 'University Applied': 'border-orange-300',
    'Admission Received': 'border-yellow-300', 'Scholarship Awarded': 'border-lime-300',
    'Visa Preparation': 'border-cyan-300', 'Visa Interview': 'border-teal-300',
    'Visa Approved': 'border-green-300', 'Travel Completed': 'border-emerald-400',
  }
  const stageHeaderColors: Record<string, string> = {
    'New Lead': 'bg-slate-100 text-slate-700', 'Contacted': 'bg-blue-100 text-blue-700',
    'Consultation Scheduled': 'bg-indigo-100 text-indigo-700', 'Documents Requested': 'bg-purple-100 text-purple-700',
    'Documents Received': 'bg-violet-100 text-violet-700', 'University Applied': 'bg-orange-100 text-orange-700',
    'Admission Received': 'bg-yellow-100 text-yellow-700', 'Scholarship Awarded': 'bg-lime-100 text-lime-700',
    'Visa Preparation': 'bg-cyan-100 text-cyan-700', 'Visa Interview': 'bg-teal-100 text-teal-700',
    'Visa Approved': 'bg-green-100 text-green-700', 'Travel Completed': 'bg-emerald-100 text-emerald-800',
  }
